...
interface HomeProps {
  user: User | null;
  hero: TMDBMovie | null;
  sections: { title: string; movies: TMDBMovie[] }[];
  error: string | null;
}
...
export const getServerSideProps: GetServerSideProps<HomeProps> = async (context) => {
  ...
  try {
    ...
    const hero =
      allMoviesForHero[Math.floor(Math.random() * allMoviesForHero.length)] ??
      null;

    const sections = [
      { title: "Random Picks", movies: sampleMovies(popular.results, 15) },
      { title: "Trending This Week", movies: sampleMovies(trending.results, 15) },
      { title: "Top Rated", movies: sampleMovies(topRated.results, 15) },
      { title: "Now Playing", movies: sampleMovies(nowPlaying.results, 15) },
    ];

    return { props: { user, hero, sections, error: null } };
  } catch (e) {
